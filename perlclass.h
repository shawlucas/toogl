...
template <class T> class PerlListBase {
  private:
    enum { ALLOCINC = 20 };
    T* a;
    int cnt;
...
template <class T> class PerlList : private PerlListBase<T> {
  public:
    ...
    T& operator[](const int i) {
        return PerlListBase<T>::operator[](i);
    }
    const T& operator[](const int i) const {
        return PerlListBase<T>::operator[](i);
    }
    using PerlListBase<T>::count;
    ...
    int isempty(void) const {
        return !count();
    } // for those that don't like the above (hi michael)
...
class PerlString {
  ...
  public:
    ...
    PerlString& operator=(const char* s) {
        pstr = s;
        return *this;
    }
    ...
    int operator<(const PerlString& s) const {
    ...
    PerlString& operator+=(const PerlString& s) {
        pstr.add(s);
        return *this;
    }
    PerlString& operator+=(const char* s) {
...
// Streams operators
template <class T> std::istream& operator>>(std::istream& ifs, PerlList<T>& arr) {
    T a;
...
template <class T> std::ostream& operator<<(std::ostream& os, const PerlList<T>& arr) {
...
std::istream& operator>>(std::istream& ifs, PerlString& s);
std::istream& operator>>(std::istream& ifs, PerlStringList& sl);
std::ostream& operator<<(std::ostream& os, const PerlString& arr);
std::ostream& operator<<(std::ostream& os, const PerlStringList& arr);

// Implementation of template functions for perllistbase
template <class T> INLINE T& PerlListBase<T>::operator[](const int i) {
...
template <class T> INLINE const T& PerlListBase<T>::operator[](const int i) const {
    assert((i >= 0) && (i < cnt));
    return a[first + i];
}

template <class T> PerlListBase<T>::PerlListBase(const PerlListBase<T>& n) {
    ...
    a = new T[allocated];
    for (int i = 0; i < cnt; i++)
        a[first + i] = n.a[first + i];
#ifdef DEBUG
    fprintf(stderr, "PerlListBase(PerlListBase&) a= %p, source= %p\n", a, n.a);
#endif
}

template <class T> PerlListBase<T>& PerlListBase<T>::operator=(const PerlListBase<T>& n) {
...
template <class T> void PerlListBase<T>::grow(int amnt, int newcnt) {
    if (amnt <= 0)
        amnt = allocinc; // default value
    if (newcnt < 0)
        newcnt = cnt; // default
    allocated += amnt;
...
template <class T> void PerlListBase<T>::add(const T& n) {
    if (cnt + first >= allocated)
        grow();
    a[first + cnt] = n;
    cnt++;
}

template <class T> void PerlListBase<T>::add(const int ip, const T& n) {
    assert(ip >= 0);
    if (first == 0 || (first + cnt) >= allocated)
        grow();
...
template <class T> void PerlListBase<T>::compact(const int n) { // shuffle down starting at n
    int i;
    assert((n >= 0) && (n < cnt));
    if (n == 0)
        first++;
...
template <class T> void PerlList<T>::push(const PerlList<T>& l) {
    for (int i = 0; i < l.count(); i++)
        add(l[i]);
}

template <class T> int PerlList<T>::unshift(const PerlList<T>& l) {
    for (int i = l.count() - 1; i >= 0; i--)
        unshift(l[i]);
    return count();
}

template <class T> PerlList<T> PerlList<T>::reverse(void) {
    PerlList<T> tmp;
...
template <class T> PerlList<T> PerlList<T>::sort(void) {
    PerlList<T> tmp(*this);
    int n = tmp.scalar();
...
template <class T> PerlList<T> PerlList<T>::splice(int offset, int len, const PerlList<T>& l) {
...
template <class T> PerlList<T> PerlList<T>::splice(int offset, int len) {
    PerlList<T> r;
...
template <class T> PerlList<T> PerlList<T>::splice(int offset) {
    PerlList<T> r;
